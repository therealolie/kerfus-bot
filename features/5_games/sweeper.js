...
function genboard(size,mineamount,mineChance){
	let board = [];
...
exports.run = (msg, client) => {
	...
	case 'start':{
		...
		boatd = genboard(size,mineamount,args[0]);
		player = {};
		player.board = board;
		player.mta = args[0];
	...
	case 'help':{
		msg.reply(
		'welcome to the complex minesweeper!\n'+
	...
	if(player=={})
		...
		client.db.v2setuser("sweeper",msg.author.id,JSON.stringify(player))
}
