...
exports.run = (msg) => {
	if(!pastas)
		pastas = new (require('../../funcs/pastas.js'))("copypastas.json", msg.client, true);
	if(!msg.content.startsWith('!stop-pasta'))
		return;
	...
	if(!msg.client.data?.channels?.[msg.channel.id]?.lastpasta){
		msg.reply(
			'no copypastas sent in this channel,'+
			'perhaps the bot restarted (or just my'+
			'spagetthi code not working)')
		return;
	}
